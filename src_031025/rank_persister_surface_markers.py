...
# Canonical gene symbols -> accepted symbols in older reference annotations.
# Important for the 39-donor AML cohort: some donors use the historical
# symbol PVRL2 for NECTIN2 (same Ensembl gene ENSG00000130202).
GENE_SYMBOL_ALIASES = {
    "NECTIN2": ["NECTIN2", "PVRL2"],
}

# Current vendor-backed live-cell flow targets verified against official BD/BioLegend
# resources (Sep 2026). This is intentionally conservative and separate from HPA
# surface annotation. Update only after re-verifying vendor availability.
VENDOR_VERIFIED_FLOW_GENES = {
    "FLT3",      # CD135
    "CD33",
    "NECTIN2",   # CD112
    "CD44",
...
def load_mtx_candidates(
    ...
    for g in candidate_genes:
        # Accept legacy/reference-specific symbols for the same gene.
        aliases = GENE_SYMBOL_ALIASES.get(g, [g])
        js = []
        for alias in aliases:
            js.extend(idx_by_gene.get(alias, []))
        # Deduplicate column indices in case aliases resolve to the same feature.
        js = sorted(set(js))
        if not js:
            continue
        if len(js) == 1:
...
def final_panel_from_full_data(
    ...
    n_final: int,
    redundancy_threshold: float,
    preferred_panel_size: int = 6,
) -> pd.DataFrame:
    """Build a stable final panel from genes actually selected in donor-held-out CV.

    Selection frequency from the preferred panel size (default: 6) is the primary
    stability criterion. The function never pads with genes that were not selected
    in CV and never leaves the caller-supplied candidate pool.
    """
    st = full_stats.copy()
    if st.empty:
        return st

    # Use the experimentally relevant panel size for stability.
    if not selection.empty:
        available_sizes = sorted(selection["panel_size"].dropna().unique().tolist())
        if preferred_panel_size in available_sizes:
            freq_panel_size = preferred_panel_size
        else:
            valid = [s for s in available_sizes if s <= n_final]
            freq_panel_size = max(valid) if valid else max(available_sizes)

        sel = selection[selection["panel_size"] == freq_panel_size].copy()
        freq = sel.groupby("gene")["fold"].nunique()
        denom = max(1, sel["fold"].nunique())
        st["cv_selection_frequency"] = st["gene"].map(freq).fillna(0.0) / denom
    else:
        freq_panel_size = None
        st["cv_selection_frequency"] = 0.0

    # Strict eligibility: only genes selected in >=1 CV fold.
    eligible = st[st["cv_selection_frequency"] > 0].copy()
    if eligible.empty:
        print(
            "[final-panel][WARNING] No genes had positive CV selection "
            "frequency; returning empty final panel."
        )
        return st.iloc[0:0].copy()

    # CV stability is primary; transcript-level surrogate strength is secondary.
    eligible["final_priority"] = (
        0.60 * eligible["cv_selection_frequency"]
        + 0.40 * eligible["surrogate_score"]
    )
    eligible = eligible.sort_values(
        [
            "cv_selection_frequency",
            "final_priority",
            "surrogate_score",
            "median_auc_oriented",
        ],
    ...
    n_cap = min(int(n_final), len(eligible))

    # Final redundancy pruning on the eligible pool only.
    donors = sorted(donor_frames)
    pool = eligible["gene"].tolist()
    corr = compute_redundancy_matrix(donor_frames, donors, pool)
    ...
    out = eligible[eligible["gene"].isin(selected)].copy()
    order_map = {g: i + 1 for i, g in enumerate(selected)}
    out["final_panel_order"] = out["gene"].map(order_map)
    out["cv_frequency_panel_size"] = freq_panel_size
    out["vendor_search_aliases"] = out["gene"].map(
        lambda g: ";".join(KNOWN_VENDOR_ALIASES.get(g, [g]))
    )

    return out.sort_values("final_panel_order").reset_index(drop=True)
...
def parse_args():
    ...
    ap.add_argument(
        ...
        help="Restrict panel selection and donor-held-out CV to a conservative set "
             "of current BD/BioLegend-supported live-cell flow targets. This is "
             "stricter than --surface-only and is intended for experimental panel design."
    )
    ap.add_argument(
...
def main():
    ...
    panel_genes = genes
    if args.flow_only:
        flow = [g for g in genes if g in VENDOR_VERIFIED_FLOW_GENES]
        print(f"\n[flow-only] {len(flow)}/{len(genes)} candidates are in the current "
              "BD/BioLegend-verified live-cell flow set.")
        print("[flow-only] genes: " + ";".join(flow))
        if len(flow) < 6:
            raise RuntimeError(
                f"--flow-only left only {len(flow)} genes; fewer than 6 are present in "
                "the candidate universe. Re-verify/add vendor-backed targets before panel CV."
            )
        panel_genes = flow
        full_stats["flow_panel_eligible"] = full_stats["gene"].isin(set(flow))
        full_stats.to_csv(out_dir / "surface_marker_ranked_all.csv", index=False)
        print(f"[flow-only] panel selection & CV restricted to these {len(flow)} genes.")
    ...
    # Final panel must be drawn strictly from the same pool supplied to CV.
    final_stats_pool = full_stats[
        full_stats["gene"].isin(set(panel_genes))
    ].copy()

    # In flow-only mode, six markers are the practical design point because the
    # held-out performance plateaued beyond six in the current analysis.
    preferred_final_size = 6 if args.flow_only else args.final_markers

    final = final_panel_from_full_data(
        full_stats=final_stats_pool,
        donor_frames=donor_frames,
        selection=selection,
        n_final=preferred_final_size,
        redundancy_threshold=args.redundancy_threshold,
        preferred_panel_size=6,
    )
    ...
    summary = {
        ...
        "flow_only_mode": bool(args.flow_only),
    ...
    print(
        "\nNEXT STEP: validate the final flow-compatible marker set at protein level, "
        "then choose clones/fluorochromes and design the practical staining panel "
        "with AML/backbone gating kept separate."
    )
