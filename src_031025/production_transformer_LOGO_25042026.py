...
from sklearn.model_selection import StratifiedKFold, GroupShuffleSplit, StratifiedShuffleSplit, StratifiedGroupKFold
...
def stratified_kfold_validation(X, y, sids, n_folds=3):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y, groups=sids), 1):
...
def main():
    ...
    threshold, oof_probs = stratified_kfold_validation(X_train, y_train, sids[train_idx], n_folds=3)
    ...
    callbacks = [
        ...
        ModelCheckpoint('best_model_logo.h5', monitor='val_pr_auc', mode='max', save_best_only=True)
    ]
    
    # Validation split
    val_split = StratifiedShuffleSplit(n_splits=1, test_size=0.15, random_state=SEED)
    tr_idx, val_idx = next(val_split.split(X_train_p, y_train, groups=sids[train_idx]))
    ...
    # Save everything
    model.save('final_model_logo.h5')
    joblib.dump(scaler, 'scaler.pkl')
    joblib.dump(pca, 'pca_logo.pkl')
    joblib.dump(float(threshold), 'threshold.pkl')
    
    with open('metadata_logo.json', 'w') as f:
        json.dump({
    ...
    print("Files saved: final_model_logo.h5, scaler.pkl, pca_logo.pkl, threshold.pkl")
    print("="*60)
