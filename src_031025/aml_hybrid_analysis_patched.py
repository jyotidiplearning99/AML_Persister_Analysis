...
MODULES = {
    ...
    # Proliferation (high = cycling, low = quiescent)
    'proliferation': [
        'MKI67',  # Ki-67 — cycling cells
        'TOP2A',  # topoisomerase — S/G2
        'PCNA',   # DNA replication
        'MCM2',   # replication licensing
        'CDK1',   # mitosis kinase
    ],
...
def score_modules(adata):
    ...
    # Quiescence proxy: inverse of proliferation
    adata.obs['score_quiescence_proxy'] = (
        adata.obs['score_quiescence'] -
        adata.obs['score_proliferation']
    )
...
def test_co_occurrence(adata, condition_col, condition_a, condition_b,
    ...
    pairs = [
        ...
        ('score_hybrid',            'score_proliferation', 'Hybrid vs Proliferation'),
    ]
...
def plot_all(adata_dict, comparison_pairs, stats_df, cooccurrence_df, output_dir):
    ...
    score_cols = ['score_senescence_arrest', 'score_sasp', 'score_stemness',
                  'score_proliferation', 'score_dna_damage', 'score_hybrid']
